---
title: "DATA698 Research Project"
...
We read in longevity, obesity, and physical activity data (sourced from the IHME), select our columns of interest and then proceed to explore and normalize our variables:
...
life_table <- life_table %>% select(1:2,13:16) %>% na.omit()
...
obesity_table <- obesity_table %>% select(1:2,5:6,9:10) %>% na.omit()
...
act_table <- act_table %>% select(1:2,5:6,9:10) %>% na.omit()

```

With data read in we can proceed to explore our variables and glean insight as to how we might create our dependent 'active lifestyle' variable as a combination of variables gathered.

# Dependent Variable Creation

In order to create our dependent 'active lifestyle' variable, we've first got to familiarize ourselves with the data at hand. 
...
##LEFT OFF HERE: ranked output
##could also consider a larger group (ie. 100) to then look at the count per state ...
...
Put into compatible form with dependent variable.
Merge df's
...
#Merge dfs
#merged_df <- merge(life_table, obesity_table, by=c("State","County"))
#merged_df <- merge(merged_df, act_table, by=c("State","County"))
...
#normalization of life_table df:
##n_life <- normalize(life_table)
##head(n_life)

#visualization of life_table feature distributions
##ggplot(gather(n_life, variable, value), aes(x=value)) + stat_density() +
