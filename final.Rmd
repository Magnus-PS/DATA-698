...
Based on our R^2 value, we elect to proceed with `model_2` (our transformed model).
...
#combe datasets so we don't have to make features twice
train$dataset <- 'train'
test$dataset <- 'test'
...
We'll first attempt to optimize this model further via automatic methods (ie. StepAIC) and then apply our models to test data, compare the output statistics, and draw our conclusions and observations for next steps.
...
model_3 <- lm(formula = health_score ~ Mortality + College + FoodInsecurity + 
    ...
    data = train2)
summary(model_3) #R^2 = 0.6734, vars = 17
