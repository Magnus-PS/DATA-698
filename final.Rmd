...
With a high rate of avoidable deaths and chronic disease as well as an obesity rate at two times higher than the OECD average, the forecast for American health is gloomy. One way of correcting this trend may lie in informing the public, clarifying priorities, and then modeling / mirroring the areas across the United States that are living long and living well. With that in mind, the purpose of this project is ultimately to answer three questions: 1) What United States counties are most favorable for an active, healthy lifestyle? 2) What are the differentiating characteristics that make them so? and 3) What might the best regression model be for modeling the relationship between our healthy lifestyle metric and these differentiating characteristics?

Our methodology for addressing these questions consisted of three separate phases: Data Gathering & Preprocessing, Data Exploration & Preparation, and Multivariate Regression Analysis. We created our dependent health score variable as a combination of life expectancy, obesity, and physical activity data read in from the IHME website and then utilized our dependent variable to identify the top 10 healthiest counties (6 Colorado, 2 California, 1 Utah, and 1 Wyoming county). 

Based on these counties, we extended assumptions regarding variables to especially consider, gathered typical and atypical independent variables, and then brought all data into a consistent format to form a “master” dataset. We then operated upon our dataset and identified our strongest and weakest predictors (strongest predictors included: Mortality, College, FoodInsecurity, LTHighSchool, Sun, EQI, Income, PopChg,  Bng, and UnemploymentChg), and proceeded toward model-building with only 10 of the original 24 independent variables (due to multicollinearity and weak target-correlation).

Through normalization, feature creation, and AIC-optimization, we optimized on the coefficient of determination and generated our transformed linear regression model. We then tested the efficacy of this model against linear regression models drawn from raw and AIC-optimized data and regularized (ridge and lasso) regression models drawn from both our transformed and raw data. We output performance metrics for all models in a table, where the R-squared and RMSE values were used as part of our selection criterion and found the linear regression model applied to transformed data to be our strongest model, largely due to its stand out R-squared value of 0.71 scored on unseen data.

**Key Words: *Feature Creation, Feature Selection, County Level Health, Health Outcomes, Regression, Regularization***
...
Source <- c('Bhardwaj, Amiri, Buchwald, and Amram', 'Kaminsky, Lessler, Sharfstein, and Wallace', 'Azevedo, Hallal, Wells, and Victoria', 'An, Li, and Jiang', 'Holick and Wacker', 'Bremberg', 'Ahern, Brown, and Dukas', 'Ferreira, Jardim, Jardim, Sousa, and Rosa', 'Islam, Leer, Teo, et al.', 'Hammarstrom, Janlert, and Winefield', 'Bhardwaj, Amiri, Buchwald, and Amram')
Type <- c('Dependent', 'Dependent', 'Dependent', 'Independent', 'Independent', 'Independent', 'Independent', 'Independent', 'Independent', 'Independent', 'Independent')
Variable <- c('Longevity', 'Obesity', 'Physical Activity', 'EQI (Environmental Quality)', 'Sunlight', 'Population Density', 'Nutrition', 'Alcohol Consumption', 'Income', 'Unemployment', 'Education')
...
Before doing so, we utilized the **Boruta** function for feature ranking and selection as confirmation (included in Appendix). In doing so, `MortalityChg`, `BngPctChg`, `Unemployment`, `HvyPctChg`, and `Population` were identified as the variables with the lowest importance. 
...
Our first ridge regression model was trained on raw data while the second was trained on transformed data. We then computed the optimal lambda for each model and evaluated the models (with optimal lambda as a parameter). *Performance metrics are noted in the 'Model Selection' section.*
...
Our first LASSO model was trained on raw data while the second was trained on transformed data. We then computed the optimal lambda for each model and evaluated the models (with optimal lambda as a parameter). *Performance metrics are noted in the 'Model Selection' section.*
