...
For our approach, we first got to know the structure and value ranges, we then looked at the distributions of our features, visualized the relationship our variables had with one another and the target variable via correlation matrix, and visited the proportion of missing data vs. correlation with target. After this point, we'd drawn enough insight to prepare our data and build our model(s).
...
We explored linear regression models with raw data and AIC-optimized data, and then proceeded to use regularization (ridge and lasso) regression models on both our transformed and raw data.

LEFT OFF HERE: comment on the purpose of this section (to explore different models and then select the strongest)

### Linear Regression (Raw)

Comment on why all variables and why AIC optimized.
...
Comment on why regularization, what ridge regression was and what the precursor steps were ...
...
Transition: closeout discussion of different models.

### Model Selection

We output the performance metrics of all models in a table to select the model with the strongest predictive promise. We evaluate performance using the R-squared value and Root Mean Squared Error (RMSE). Lower RMSE and higher R-squared values are indicative of a stronger model.

To succinctly summarize our model optimization attempts and results, we capture the method used, the data the method was applied to, as well as the number of variables considered, and corresponding evaluation metrics for raw and transformed data:
